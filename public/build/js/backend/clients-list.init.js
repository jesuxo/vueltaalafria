...
var clientList = new List("clientList", options).on("updated", function (list) {
...
xhttp.onload = function () {
    var json_records = JSON.parse(this.responseText);
        ...
        Array.from(json_records).forEach(function (element) {
            ...
            clientList.sort('id', { order: "desc" });

        });

        clientList.remove("id", `<a href="javascript:void(0);" class="fw-medium link-primary">#VZ01</a>`);
}
xhttp.open("GET", "saclie/json");
xhttp.send();
...
isCount = new DOMParser().parseFromString(
    clientList.items.slice(-1)[0]._values.id,
    "text/html"
);
